...
const ProductImageGallery = ({ images }: ProductImageGalleryProps) => {
  ...
  if (!images || images.length === 0)
    return <div className="aspect-square bg-muted flex items-center justify-center text-6xl">📦</div>;
    ...
    <div className="w-full">
      ...
      <div className="hidden lg:flex flex-col space-y-4">
        {images.map((img, index) => (
          <div key={index} className="w-full max-h-[500px] overflow-hidden gap-2 cursor-pointer" onClick={() => setCurrentIndex(index)}>
            <img src={img} alt={`Product view ${index + 1}`} className="w-full h-full object-contain bg-muted transition-transform duration-300" />
          </div>
        ))}
      </div>
      ...
      <div className="lg:hidden relative">
        <div className="w-full aspect-square overflow-hidden cursor-pointer"
          ...
          <img src={images[currentIndex]} alt={`Product view ${currentIndex + 1}`} className="w-full h-full object-contain bg-muted transition-transform duration-300 select-none" />
        </div>
        <div className="flex justify-center mt-4 gap-2">
          {images.map((_, i) => (
            <button key={i} className={`w-2 h-2 rounded-full transition-colors ${i === currentIndex ? "bg-foreground" : "bg-muted"}`} onClick={() => setCurrentIndex(i)} />
          ))}
        </div>
      </div>
    </div>
...
export default function ProductDetails() {
  ...
  const { data: seller } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("profiles").select("*").eq("user_id", product!.user_id).single();
      if (error) throw error;
      return data;
    },
  });
  ...
  const handleMessageSeller = async () => {
    ...
    const { data: existing } = await supabase.from("conversations")
      .select("id").eq("product_id", product.id).eq("buyer_id", user.id).maybeSingle();
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="lg:sticky lg:top-6 h-fit space-y-5">
            ...
            <div className="flex items-center gap-3">
              <span className="text-3xl font-bold text-primary">GHS {Number(product.price).toFixed(2)}</span>
  ...
  <Card className="p-4 flex items-center gap-3">
    ...
    />
    <div>
      <p className="text-sm font-medium">{seller.display_name || "Anonymous"}</p>
